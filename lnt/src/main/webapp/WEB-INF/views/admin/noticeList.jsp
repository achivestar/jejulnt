                        ...
                        <div class="card-body">
						    ...
						    <c:if test="${empty list}">
						      ...
						      <tr>
						        ...
						        <td>${list.subject }</td>
						        <td>${list.regdate }</td>
						        <td><a href="${pageContext.request.contextPath}/admin/noticeModify?idx=${list.idx}" class="btn btn-warning">수정</a>&nbsp;
					  ...
					  <ul class="pagination justify-content-center">
						  <c:if test="${prev}">
						    <li class="page-item disabled">
						      <a class="page-link" href="./noticeList?pnum=${startPageNum - 1}" tabindex="-1">Previous</a>
						    </li>
					     </c:if>
					      <c:forEach begin="${startPageNum}" end="${endPageNum}" var="pnum">
					      	  <c:if test="${select != pnum}">
					      	  <li class="page-item"><a class="page-link" href="./noticeList?pnum=${pnum}">${pnum}</a></li>
					      	  </c:if>
					      	  <c:if test="${select == pnum}">
					      	   <li class="page-item active">
					    		  <a class="page-link"href="./noticeList?pnum=${pnum}">${pnum } <span class="sr-only">(current)</span></a>
					    		</li>
					      	  </c:if>
					      </c:forEach>
					      <c:if test="${next}">
					        <li class="page-item">
					    		  <a class="page-link" href="./noticeList?pnum=${endPageNum + 1}">Next</a>
						    </li>
					      </c:if>
